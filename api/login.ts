/** GET /api/login  — session check or ?action=logout
 *  POST /api/login — authenticate user */
import { supabase } from "../lib/supabase";
import { parseSession, createSessionCookie, destroySessionCookie } from "../lib/session";
...
export default async function handler(req: Request): Promise<Response> {
  // GET: session check or logout
  if (req.method !== "POST") {
    const url = new URL(req.url);
    if (url.searchParams.get("action") === "logout") {
      return new Response(JSON.stringify({ success: true }), {
        headers: { "Content-Type": "application/json", "Set-Cookie": destroySessionCookie(req) },
      });
    }
  ...
  if (!hasEnv("SUPABASE_URL") || !hasEnv("SESSION_SECRET") || (!hasEnv("SUPABASE_SERVICE_KEY") && !hasEnv("SUPABASE_ANON_KEY")))
    return json({ error: "Authentication is not configured on the server." }, { status: 503 });
  ...
  if (password.length > 128) return json({ error: "Invalid credentials" }, { status: 401 });

  const { data: user } = await supabase.from("users").select("id,email,name,role,password_hash,failed_attempts,locked_until").eq("email", email).maybeSingle();

  // Account lock check
  if (user?.locked_until && Date.now() < Number(user.locked_until)) {
    const remaining = Math.ceil((Number(user.locked_until) - Date.now()) / 60000);
    return json({ error: `Account locked. Try again in ${remaining}m.`, locked: true }, { status: 423 });
  }
  ...
  if (!user || !valid) {
    if (user) {
      ...
      if (lockUntil) return json({ error: "Account locked for 10 minutes.", locked: true }, { status: 423 });
      const left = 5 - attempts;
      if (left <= 2) return json({ error: `Invalid credentials. ${left} attempt${left !== 1 ? "s" : ""} remaining.` }, { status: 401 });
    }
