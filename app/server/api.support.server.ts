...
export async function action({ request }: { request: Request }) {
  ...
  // IMPORTANT: If upload fails for ANY reason, we still insert the ticket.
  // Screenshot is fully optional — never block ticket creation on it.
  ...
  if (insertErr) {
    // Return the REAL Supabase error message so we can debug it directly
