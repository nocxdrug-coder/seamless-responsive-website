...
export async function loader({ request }: { request: Request }) {
  try {
    ...
    const { getUserOrders, getProductById, ensureSeeded } = await import("~/server/db.server");
    const { requireSession } = await import("~/server/session.server");
    ...
    // Re-throw intentional Response returns (e.g. 401 from requireSession)
    if (err instanceof Response) throw err;
    console.error("[api/orders] loader error:", err);
    return json({ orders: [] });
  }
}
